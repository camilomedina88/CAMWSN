...
static class CammQueueClass : public TclClass {
public:
        CammQueueClass() : TclClass("Queue/Camm") {}
        TclObject* create(int argc, const char*const* argv) {
	         return (new CammQueue(atoi(argv[4])));
	}
} class_dropt_tail_round_robin;


void CammQueue::enque(Packet* p)
{
...
//ESTADO REJECT
  if(tamanioActual > qlim_*0.80) estadoBuffer=2;
//ESTADO ACEPT
  if(tamanioActual < qlim_*0.60) estadoBuffer=0;
//ESTADO FILTER
  if(tamanioActual >= qlim_*0.6 && tamanioActual <= qlim_*0.80)     estadoBuffer=1;   
...
void CammQueue::sortQueue(PacketQueue* queueActual){
