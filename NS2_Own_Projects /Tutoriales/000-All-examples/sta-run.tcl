...
set fh_throughput "sta-afr_throughput.txt"
set fh_a_delay    "sta-afr_average_delay.txt"
set fh_p_delay    "sta-afr_peak_delay.txt"
set fh_fairness   "sta-afr_fairness.txt"
...
for {set i 0} {$i < [llength $BER] } {incr i} {
    for {set k 0} {$k < [llength $nn] } {incr k} {
    	    exec ns sta-afr-david.tcl [lindex $BER $i] [lindex $PKT $i] [lindex $Frame $i] [lindex $Frag $i] [lindex $DelayTimer 0] [lindex $nn $k] >> "auto-result.txt" 2> /dev/null
    }
    new_a_line
}
