...
Application/BSApp instproc recvINFO {sender msg} {
  ...
  set E [lindex $msg 2]
  incr total_
  puts "BS received info: ($X $Y $E) from Node $sender - mit/uAMPS/ns-bsapp.tcl" 
  puts "BS received: $total_ - mit/uAMPS/ns-bsapp.tcl"
...
Application/BSApp instproc recvDATA {sender msg} {
  ...
  puts "BS Received data $msg from $sender at time [$ns_ now] - mit/uAMPS/ns-bsapp.tcl "
#   puts "PRIJEM BS "
  ...
  puts "This represents data from nodes: $actual_nodes_data - mit/uAMPS/ns-bsapp.tcl"
...
Application/BSApp instproc reset_num_clusters {} {
  global opt cas ns_
  $self instvar total_

  if {[$ns_ now] != $cas} {
    set opt(num_clusters) [expr int(ceil(double(5.0 * double($total_)/100.0)))]
    puts "Change num_clusters is now $opt(num_clusters)"
    set cas [$ns_ now]
  }
  return 
}
...
Application/BSApp instproc BSsetup {} {
  ...
  if {$total_ > $opt(num_clusters)} {
    ...
    puts "Only received info from $total_ nodes. - mit/uAMPS/ns-bsapp.tcl"
    puts "There are currently $opt(nn_) alive ==> \
          $opt(num_clusters) cluster-heads needed. - mit/uAMPS/ns-bsapp.tcl"
    "sens_finish"
  }
  set total_ 0
  ...
  if {$opt(rcapp) == "LEACH/LEACH-C" || \
    ...
    if {$opt(num_clusters) != 1} {
      $ns_ at [expr [$ns_ now] + $opt(ch_change)] "$self reset_num_clusters"
    }
