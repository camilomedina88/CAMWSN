...
# Open file for trace
set tf [open $output_dir/wimax_2RS_4SS_4SN_cenario_02a.tr w]
$ns trace-all $tf
puts "Output file configured"

# Open file for recording throughput
set throughput_rv_ [open $output_dir/throughput_2RS_4SS_4SN_cenario_02a.tr w]
...
for {set i 0} {$i < $nb_rs} {incr i} {
  ...
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 10.0 20.0 4 7    ;# start; end; MS address; DIUC
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 20.0 30.0 4 2
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 30.0 40.0 4 1
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 40.0 50.0 4 2
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 50.0 60.0 4 7
  if {$i == 0} {
  	[$rstation_($i) set mac_(0)] set-control-method 1
  } else {
	[$rstation_($i) set mac_(0)] set-control-method 0	
  }
}
