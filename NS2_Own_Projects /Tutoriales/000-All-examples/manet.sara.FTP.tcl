...
for {set i 0} {$i < $opt(sessions)} {incr i} {
    ...
    $ns_ attach-agent $node_($source) $tcp($i)
    $ns_ attach-agent $node_([expr 100 + $i]) $sink($i)
    ...
    $tcp($i) set fid_ Green
    $ns_ at $sim_st "$ftp($i) start"
    $ns_ at [expr $val(stop)-1] "$ftp($i) stop"
