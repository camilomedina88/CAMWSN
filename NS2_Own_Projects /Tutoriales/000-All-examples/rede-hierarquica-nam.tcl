...
set val(ifqlen)  25        ; # queue length 
...
set val(n_fcommon)  5      ; # number of false common nodes 
...
set val(disseminating_interval)  2.0    ; # common node disseminating interval 
set val(cluster_head_disseminating_interval)  5.0 ; # cluster head disseminating interval 

set val(start)      5.0    ; # simulation start time 
set val(stop)     35.0    ; # simulation stop time 
...
set traceFile [open mannasim-rede-hierarquica2c.tr w]
$ns_ trace-all $traceFile



$ns_ namtrace-all $traceFile
set traceFile [open mannasim-rede-hieraquica2c.nam w]
$ns_ namtrace-all $traceFile
# $ns_ namtrace-all [open mannasim-rede-hieraquica2c.nam w]
...
Phy/WirelessPhy set Pt_ $val(pt_common)
    ...
    # add by PAPE
	Node/MobileNode/SensorNode set valueMac_ 1
...
# PAPE ADDED TO TEST
            #puts " PAPE is testing and the node is node ($j) "
		}
	}
...
# ======================================== 
# Procedure to create a false common node  
# ======================================== 
proc create_common_node_2 {} {
global val ns_ node_ topo udp_ app_ gen_ contador_nodos rng
...
Phy/WirelessPhy set Pt_ $val(pt_common)
	...
	Node/MobileNode/SensorNode set processingPower 0.024
	Node/MobileNode/SensorNode set instructionsPerSecond_ 8000000
# add by set the mac key value
	Node/MobileNode/SensorNode set valueMac_ 3
	Phy/WirelessPhy set  bandwidth_ 288000.0
...
# add by pape 
	...
	set udp_($contador_nodos) [new Agent/UDP]
	# Adding a loss monitor to the udp agent by pape
		set lossmon_($contador_nodos) [new Agent/LossMonitor]
	$ns_ connect $udp_($contador_nodos) $lossmon_($contador_nodos)
	...
	#set app_($contador_nodos) [create_cluster_head_app [$node_(1) node-addr]
	$val(disseminating_type) $val(cluster_head_disseminating_interval)]
	$node_($contador_nodos) attach $udp_($contador_nodos) $val(port) 
	$node_($contador_nodos) add-app $app_($contador_nodos) 
	set processing_($contador_nodos) [new Processing/AggregateProcessing] 
...
# $mc = $i 
# create_common_node_2 
for {set i 0} {$i < $val(n_fcommon)} {incr i} {
	create_common_node_2
...
proc finish {} {
  ...
  # Call xgraph to display the results 
       # exec xgraph mannasim-rede-hierarquica.tr -geometry 800x400 & 
	exit 0
}
