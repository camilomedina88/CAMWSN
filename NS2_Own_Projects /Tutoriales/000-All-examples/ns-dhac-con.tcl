############################################################################
# ZCJ DHAC with CONNECTIVITY information (2007)
...
Class Application/DHAC-CON -superclass Application

############################################################################
# Start/End: DHAC-CON algorithm Application
############################################################################
Application/DHAC-CON instproc init args {
...
#===========================================================================
##### trigger the DHAC-CON algorithm
Application/DHAC-CON instproc start {} {
  ...
  set next_phase_time [expr $now_ + $opt(step_span) * $opt(nn) * 2] 
...
Application/DHAC-CON instproc setupRESEMBLANCETABLE {} {
  ...
  set exchange_neighborlist_time [expr $now__ + $opt(step_span) * $opt(nn_)]
  $ns_ at $exchange_neighborlist_time "$self exchangeNEIGHBORLIST"

  set setup_table_time [expr $now__ + $opt(step_span) * $opt(nn_) * 2]
  $ns_ at $setup_table_time "$self TABLEPRINTING"
}

#===========================================================================
##### exchange the HELLO message
Application/DHAC-CON instproc exchangeHELLOMESSAGE {} {
  global chan ns_ opt
...
Application/DHAC-CON instproc sendHELLO {distance} {
...
Application/DHAC-CON instproc recvHELLO {sender} {
...
#===========================================================================
##### exchange the NEIGHBORLIST message
Application/DHAC-CON instproc exchangeNEIGHBORLIST {} {
  global chan ns_ opt
  ...
  ##Send message closest_dis_ meters so the closest neighbor nodes can hear.
  set random_access [$self getRandomNumber 0 [expr $opt(ra_hello)*$opt(nn)]]
  $ns_ at [expr $now__ + $random_access] "$self sendNEIGHBORLIST $distance"
}

#===========================================================================
##### broadcast the NEIGHBORLIST message
Application/DHAC-CON instproc sendNEIGHBORLIST {distance} {
  global ns_ opt node_ neighbor
  global NEIGHBORLIST MAC_BROADCAST LINK_BROADCAST BYTES_ID
  ...
  set mac_dst $MAC_BROADCAST
  set link_dst $chID
  set msg [list $neighbor($chID)]
  set datasize [expr $BYTES_ID * [join [llength $msg]]]

  $self send $mac_dst $link_dst $NEIGHBORLIST $msg $datasize $distance $code_
  pp -file "msg" "NA\t[$ns_ now]\tsendNEIGHBORLIST\t$chID\tNA\t$NEIGHBORLIST\t$msg\t$datasize\t$distance\t$mac_dst\t$link_dst"

}

#====================================================================
# recieved the NEIGHBORLIST message
Application/DHAC-CON instproc recvNEIGHBORLIST {sender} {

  global ns_ opt node_
  global neighbor
  
  #build up the connectivity table
...
Application/DHAC-CON instproc TABLEPRINTING {} {
  global chan ns_ opt
...
############################################################################
# * 2 * SETUP PHASE: DHAC-CON Clustering phase
############################################################################
##### The main function of executing the DHAC-CON algorithm
Application/DHAC-CON instproc executeDHAC {} {
  ...
  ##SETUP PHASE (2-1): execute DHAC-CON algorithm
...
############################################################################
# * 3 * SETUP PHASE: execute DHAC-CON algorithm
############################################################################
#===========================================================================
###### SEARCH the neighbor with SMALLEST COEFFICIENT and send INVITE message
Application/DHAC-CON instproc InvitePartner {} {
  global chan ns_ opt node_
...
Application/DHAC-CON instproc sendINVITE {distance} {
  global chan ns_ opt node_
...
Application/DHAC-CON instproc recvINVITE {msg sender} {
...
Application/DHAC-CON instproc ComfirmInvitation {isCorresponding INVITEreceiver INVITEsender} {
...
Application/DHAC-CON instproc sendCONFIRM {INVITEsender mergePart1 mergePart2 distance} {
  global chan ns_ opt node_
...
Application/DHAC-CON instproc recvPOSITIVECONFIRM {msg} {
  global ns_ opt
...
Application/DHAC-CON instproc recvNEGATIVECONFIRM {msg} {
  #do nothing about it
}
...
Application/DHAC-CON instproc mergeMinimumCluster {} {
...
Application/DHAC-CON instproc sendMERGE_REQ {distance} {
  global chan ns_ opt node_
...
Application/DHAC-CON instproc recvMERGE_REQ {msg sender} {
  global ns_ cluster
...
Application/DHAC-CON instproc ClusterMaintain {} {
...
Application/DHAC-CON instproc createSchedule {} {

  global ns_ opt cluster bs
 
  $self instvar now_ now__ dist_ TDMAschedule_
...
Application/DHAC-CON instproc sendSCHEDULE {distance} {
  global ns_ opt node_
...
Application/DHAC-CON instproc recvSCHEDULE {order sender} {
  global ns_ opt
...
Application/DHAC-CON instproc StartBackupCH {} {
...
Application/DHAC-CON instproc ReAssign {currentCH nodeID} {
...
Application/DHAC-CON instproc AutoChange {currentCH nodeID} {
  global chan ns_ opt node_ cluster
...
Application/DHAC-CON instproc CHECK_ENERGY_LEVEL {currentCH} {
...
Application/DHAC-CON instproc collectionDATA {order} {
...
Application/DHAC-CON instproc sendDATA {} {
...
Application/DHAC-CON instproc recvDATA {msg} {
...
Application/DHAC-CON instproc SendDataToBS {} {
...
Application/DHAC-CON instproc SendMyDataToBS {nodeID} {
...
# Receiving Functions
############################################################################
Application/DHAC-CON instproc recv {args} {
  ...
  switch $msg_type {
     ...
     1 {;#NEIGHBORLIST
       if {[lsearch $msg $nodeID] != -1} {
         $self recvNEIGHBORLIST $msg
         pp -file "msg" "NA\t[$ns_ now]\trecvNEIGHBORLIST\t$sender\
                         \t$nodeID\t$msg_type\t$msg\t$data_size\tNA\tNA\t$chID"
       }			 
     }
     2 { ;#INVITE
...
# Sending Functions
############################################################################
Application/DHAC-CON instproc send {mac_dst link_dst type msg
                                      data_size dist code} {
  global ns_
  $self instvar rng_
...
Application/DHAC-CON instproc send_now {mac_dst link_dst type msg \
                                          data_size dist code} {
...
Application/DHAC-CON instproc getRandomNumber {llim ulim} {
  $self instvar rng_
  return [$rng_ uniform $llim $ulim]
}

Application/DHAC-CON instproc node {} {
  return [[$self agent] set node_]
}

Application/DHAC-CON instproc nodeID {} {
  return [[$self node] id]
}

Application/DHAC-CON instproc mac {} {
  return [[$self node] set mac_(0)]
}

Application/DHAC-CON instproc getX {} {
  return [[$self node] set X_]
}

Application/DHAC-CON instproc getY {} {
  return [[$self node] set Y_]
}

Application/DHAC-CON instproc getER {} {
  set er [[$self node] getER]
  return $er
}

Application/DHAC-CON instproc setCode code {
  $self instvar code_
  set code_ $code
  [$self mac] set code_ $code
}

Application/DHAC-CON instproc GoToSleep {} {
  global opt ns_
...
Application/DHAC-CON instproc WakeUp {} {
  global opt ns_
  $self instvar begin_idle_ begin_sleep_
...
Application/DHAC-CON instproc checkAlive {} {
...
Application/DHAC-CON instproc isClusterHead? {nodeID} {
  global CHsign

  return [lindex $CHsign($nodeID) 0]
}

Application/DHAC-CON instproc getCurrentCH {nodeID} {
  global CHsign

  return [lindex $CHsign($nodeID) 1]
}

Application/DHAC-CON instproc setClusterHead {nodeID} {
  global CHsign

  set CHsign($nodeID) [list 1 $nodeID]
}

Application/DHAC-CON instproc unsetClusterHead {nodeID currentCH} {
  global CHsign
...
Application/DHAC-CON instproc ChangeCH {nodeID currentCH} {
  global CHsign

  set CHsign($nodeID) [list 0 $currentCH]
}

Application/DHAC-CON instproc RemoveMember {memberlist nodeID} {
