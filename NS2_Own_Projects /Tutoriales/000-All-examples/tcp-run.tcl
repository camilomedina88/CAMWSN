...
set fh_throughput "tcp_afr_throughput.txt"
set fh_a_delay    "tcp_afr_average_delay.txt"
set fh_p_delay    "tcp_afr_peak_delay.txt"
set fh_fairness   "tcp_afr_fairness.txt"
...
for {set i 0} {$i < [llength $BER] } {incr i} {
    for {set j 0} {$j < [llength $Frag] } {incr j} {
        for {set k 0} {$k < [llength $DelayTimer] } {incr k} {
    	    exec ns tcp-afr-david.tcl [lindex $BER $i] [lindex $PKT $i] [lindex $Frame $i] [lindex $Frag $j] [lindex $DelayTimer $k] >> "auto-result.txt" 2> /dev/null
        }
    }
    new_a_line
}
