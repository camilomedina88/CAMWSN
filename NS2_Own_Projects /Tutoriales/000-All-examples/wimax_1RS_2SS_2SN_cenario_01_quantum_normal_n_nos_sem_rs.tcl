...
#Mac/802_16 set frame_duration_	     0.0025
Mac/802_16 set throughput_delay_ 0.15 ;# delay máximo p/ aplicação => 150 ms
#Mac/802_16 set delay_avg_alpha 0.002
#Mac/802_16 set jitter_avg_alpha_ 0.0001
#Mac/802_16 set throughput_avg_alpha_ 1.5
#Mac/802_16 set loss_avg_alpha_ 0.002
...
Mac/802_16 set min_th_ 20 ;# MR_min padrão 1
Mac/802_16 set max_th_ 30 ;# MR_max padrão 2
...
WimaxScheduler/BS set quantum_original_ 0
WimaxScheduler/RS set quantum_original_ 0
WimaxScheduler/SS set quantum_original_ 0
...
# Create God
create-god [expr ($nb_mn + $nb_sn + 1)]				;# nb_sn + nb_mn + 1 (base station)
...
set cmd2
set cmd2 [concat $cmd2 " [expr ($nb_mn+1)]"]
#puts $cmd2
eval $cmd2
...
for {set i 0} {$i < $nb_sn} {incr i} {
  $ns duplex-link $sourceNode_($i) $bstation 100Mb 0.0002ms RED
}
...
for {set i 0} {$i < $nb_mn} {incr i} {
  set wl_node_($i) [$ns node $nb_sn.0.[expr $i + 1]] 	;# create the node with given @.
  ...
  #puts "wireless node $i created and its' channel is [expr $i/10+1]"			;# debug info
...
$vbr_($i) set rate_ 144Kb ;# corresponds to interval of 3.75ms
$vbr_($i) set rate_dev_ 0.25;
