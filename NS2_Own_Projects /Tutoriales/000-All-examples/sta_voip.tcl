...
set val(AppSize)        [lindex $argv 1] ;# 160 bytes + 40 headers (RTP+UDP+IP), I use 256
...
#------------------------------------------------------------------------
puts "----------------------------------------------------------------"
puts "BER       AppRate   Pkt     Frame    Frag    STA    Sq     D_Timer  Run"
puts "$val(BER)  $val(AppRate)    $val(PktSize)    $val(FRAMELEN)     $val(fgLEN)    $val(nn)    $val(Sq_LIMIT)   $val(DELAY)   $val(run)"
puts "----------------------------------------------------------------"
...
Mac/802_11      set recvFrameNUM_        0
Mac/802_11      set numExceed30ms_        0
Mac/802_11      set percentage_        0.015
...
#    $ns_ flush-trace
    ...
    set f_throughput [open "sta_voip_afr_throughput.txt" "a"]
    set f_a_delay [open "sta_voip_afr_average_delay.txt" "a"]
    set f_p_delay [open "sta_voip_afr_peak_delay.txt" "a"]
    set f_fairness [open "sta_voip_afr_fairness.txt" "a"]
    set f_percentage [open "sta_voip_afr_percentage.txt" "a"]
...
#	puts "--------------------------------------------------------"
    ...
    set aaa [expr $numExceed30ms.0/$recvNumber]

# puts "numExceed30ms=$numExceed30ms"
# puts "recvNumber=$recvNumber"
# puts "percentage=$aaa"
...
#	puts "The total Delay=$totalDelay"
    ...
    puts -nonewline $f_percentage "[string range $aaa 0 6] "
