...
if {$argc != 4} {
	puts ""
	puts "Wrong Number of Arguments! 4 arguments for this script"
	puts "Usage: ns datarate.tcl modulation cyclic_prefix rtPS_scheduler BE_scheduler"
...
#open file for trace
set tf [open $output_dir/BE_out.res w]
$ns trace-all $tf
...
#### interval_ ########
set interval_ugs(1) 0.5
set interval_ugs(2) 0.5
set interval_ugs(3) 0.5

set interval_ugs(4) 0.5
set interval_ugs(5) 0.5
...
#### interval_ ########
set interval_rtPS(1) 0.4
set interval_rtPS(2) 0.4
set interval_rtPS(3) 0.4
...
if {$rtPS_scheduler_ == "WRR"} {
  ...
  $ns at 1.3 "$bs_sched set-PeerNode-WRRscheduling [expr $nb_UGS + $j + 1 - $first_rtPS] $WRR_rtPS([expr $j + 1 - $first_rtPS])"
}
...
for {set j $first_BE} {$j < [expr $first_BE + $nb_BE]} {incr j} {
set wl_node_BE($j) [$ns node 1.0.[expr $nb_UGS + $nb_rtPS + $j + 1 - $first_BE]] 	;# create the node with given @.	
$wl_node_BE($j) random-motion 0			;# disable random motion
$wl_node_BE($j) base-station [AddrParams addr2id [$bstation node-addr]] ;#attach mn to basestation
#compute position of the node
$wl_node_BE($j) set X_ [expr 650 + 5 * [expr $j + 1 - $first_BE]]
$wl_node_BE($j) set Y_ 500.0
$wl_node_BE($j) set Z_ 0.0

puts "wireless node _BE $j created"

set clas [new SDUClassifier/Dest]
[$wl_node_BE($j) set mac_(0)] add-classifier $clas
#set the scheduler for the node. Must be changed to -shed [new $opt(sched)]
set ss_sched [new WimaxScheduler/SS]
[$wl_node_BE($j) set mac_(0)] set-scheduler $ss_sched
[$wl_node_BE($j) set mac_(0)] set-channel 0
...
##set-PeerNode-SNR PeerNode SNR
$ns at 1.3 "$bs_sched set-PeerNode-SNR [expr $nb_UGS + $nb_rtPS + $j + 1 - $first_BE] $SNR_BE([expr $j + 1 - $first_BE])"
...
if {$BE_scheduler_ == "WRR"} {
  # set-PeerNode-WRRscheduling PeerNode Weight
  $ns at 1.3 "$bs_sched set-PeerNode-WRRscheduling [expr $nb_UGS + $nb_rtPS + $j + 1 - $first_BE] $WRR_BE([expr $j + 1 - $first_BE])"
}


#set data_to_send_BE($j) 30000

#$ns at $traffic_start "$ss_sched set-BandwidthBEconnections $data_to_send_BE($j)"

#$ns at $traffic_start "uplink_ftp_tcp_data $wl_node_BE($j) $j $data_to_send_BE($j)"

$ns at 13.0 "uplink_ftp_tcp $wl_node_BE($j) $j"
#################################################################################
