...
# DATE: 14 FEB 2003
# DESCRIPTION:
#   This simulation illustrates how a sensor network reacts to energy
# constraints.  As nodes exhaust their energy reserve, we should see the routing
# algorithm generate alternative routes to the data collection point.  Ns-2's
# energy consumption model utilizes color to illustrate when a node is about to
# exhaust it's energy, so the node coloring which is part of the sensor app 
# should be disabled with the DISABLE_COLORS definition in sensorapp.cc.
#
# Energy parameters used in node-config are described as follows:
...
#
# These energy parameters were based on data presented in the following paper:
#   "Energy-EfficientCommunication for Ad-Hoc Wireless Sensor Networks", R. Min 
#   and A. Chandrakasan; Nov 2001; http://www.mit.edu/~rmin/research
#
################################################################################
...
for {set i 1} {$i < $val(nn) } {incr i} {
  $ns_ at 1.0 "$app_($i) start $sensor_($i)"
}
