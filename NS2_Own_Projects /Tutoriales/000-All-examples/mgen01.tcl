...
# DATE: 28 MAR 2003
# DESCRIPTION:
#   This simulation tests whether we can use MGEN agents in sensor apps.  MGEN
# and TRPR packages are available on Protean Forge <http://pf.itd.nrl.navy.mil>.
#
# Mgen output is sent to mgen01.drc.  Parse it with the following TRPR command:
#
#   trpr mgen input mgen01.drc latency auto X output mgen01.gp 
#   gnuplot -persist mgen01.gp
#
#
################################################################################
...
set tracefd [open mgen01.tr w]
$ns_ trace-all $tracefd

set namtrace [open mgen01.nam w]
...
set sink [new Agent/MGEN]
$ns_ attach-agent $node_(26) $sink
for {set i 1} {$i < $val(nn) } {incr i} {
  set src_($i) [new Agent/MGEN]
...
$sink event {listen udp 5000}
$sink output "mgen01.drc"

$ns_ at 0.0 "$sink start"
