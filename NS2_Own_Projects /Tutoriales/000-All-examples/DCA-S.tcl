...
Agent/SHIVA set timeout-destruction					4.0
...
proc usage {} {
	puts "usage: ns DCA-S.tcl \[options\]";
	puts "options:";
	puts "* -t TopologyMainDirectory\tSet Topology Main Directory";
	puts "* -N nodes\t\t\tNumber of nodes of topology";
	puts "* -I index\t\t\tIndex for topology";
	puts "  -d\t\t\t\tDegree version only for DCA";
	puts "  -c length\t\t\tCycles Length";
	puts "  -l level\t\t\tStop Algoritmo at selected level";
	puts "Default is Dca + Shiva (cycles length 3)";
}

set pos 0;
while {[string compare [lindex $argv $pos] ""] != 0} {
	if {[string compare [lindex $argv $pos] "-d"] == 0} {
...
set dcaAlgo "DCA";
if {[Agent/DCA set degree]} {
	set dcaAlgo "${dcaAlgo}(D)";
}
...
declareModule {Agent/DCA} {Utility/CLUSTERING} "$dcaAlgo" {DCA}
if {$level > 1} {
