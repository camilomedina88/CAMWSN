if {$argc != 1 } {
  puts "Please insert the run number 0 -> 47"
  exit 1
} else {
  set r [lindex $argv 0]
}
...
set ps(2) 3000
#################################################
set val(numberOfConnections)    [expr ($nc([expr int(floor($r/3)) % 4]))]                ;# 1,2,4,8,16,24
...
set val(controller)  "802.11b"    ;# "BT2.1+EDR","802.11b" , "802.11g" , simultainous
set val(packetGenerationRate) 	[expr ($pr([expr int(floor($r/12)) % 4]))]	;#send rate during on time (bps) 100000 , 1600000 , 5000000 , 20000000 video 
...
set filename "/mnt/sda3/results/5/results/$r$val(controller)$val(agent)results.csv"
