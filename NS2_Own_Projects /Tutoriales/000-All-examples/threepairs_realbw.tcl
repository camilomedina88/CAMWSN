...
while {$i<$argc} {
	switch [lindex $argv $i] {
		...
		f4 {
			set arguments(-f4start) "[lindex $argv [expr {$i + 1}]]"
			set arguments(-f4stop) "[lindex $argv [expr {$i + 2}]]"
			set i [expr {$i + 2}]
			set have(-f4) "true"
		}
		d1 {
...
if {$have(-f4) == "false"} {
	set arguments(-f4start) 0
	set arguments(-f4stop) 16e5
	set have(-f4) "true"
}
if {$have(-d1) == "false"} {
...
if {$have(-f4) == "true"} {
	set arguments(-scenarioname) "$arguments(-scenarioname).realbw"
}
...
proc record_ {} {
	...
	set sortie "$now\t[$cbr_(7) set rate_]\t"
	foreach item [array names bw_] {
...
controlflux $cbr_(5) 10.0 [expr $val(stop)-5.0]
controlflux $cbr_(3) 15.0 [expr $val(stop)-5.0]
...
proc increase_find {starttime stoptime i} {
	...
	set ns [Simulator instance]
	set cbr7rate [expr $arguments(-f4start)+$i*($arguments(-f4stop)-$arguments(-f4start))/20]
	$cbr_(7) set rate_ $cbr7rate

	#$ns at $starttime "$cbr_(7) start"
	#$ns at $stoptime "$cbr_(7) stop"
	controlflux $cbr_(7) $starttime $stoptime

	$ns at $stoptime "$cbr_(7) set rate_ 0"
	#set nextstarttime [expr $starttime+20+[$rUni value]/1000.0]
	set nextstarttime [expr $starttime+50]
	set nextstoptime [expr $stoptime+50]
	set nexti [expr $i+1]
	if {$i<20} {
		#Re-schedule the procedure
		#$ns at [expr $stoptime+10.0] "increase_saturated $nextstarttime $nextstoptime $nexti"
