...
proc reset_num_clusters {} {
  global opt time2 ns_
  if {[$ns_ now] != $time2} {    
    set opt(num_clusters) [expr int(ceil(double(5.0 * double($opt(nn_))/100.0)))]
#     puts "Num_clusters was changed to $opt(num_clusters)"
    set time2 [$ns_ now]
  }
  return 
}
...
Application/LEACH instproc decideClusterHead {} {
  ...
  if {$opt(num_clusters) != 2} {
    $ns_ at $next_change_time_ "reset_num_clusters"
  }	
