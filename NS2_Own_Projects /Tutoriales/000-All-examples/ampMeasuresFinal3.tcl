if {$argc != 1 } {
  puts "Please insert the run number 0 -> 71"
  exit 1
} else {
  set r [lindex $argv 0]
}
...
set val(txPower_)	500 		;#1.0		;#0.00175
set val(rxPower_)	500		;#1.0		;#0.00175
set val(idlePower_)	100		;#1.0		;#0.0032
set val(sleepPower_)	50		;#0.001		;#0.00005
set val(transitionPower_)	100	;#0.2	;#0.002
...
set val(trafficStopTime) 35
set val(simulationStopTime) 80
##################################################
...
set ps(2) 3000
#################################################
set val(numberOfConnections)    [expr ($nc([expr int(floor($r/6)) % 3]))]                ;# 1,2,4,8,16,24
set val(nn)             [expr ($val(numberOfConnections) * $m([expr int(floor($r/3)) % 2]))]      ;# number of mobilenodes 8,16,24
...
set val(packetGenerationRate) 	[expr ($pr([expr int(floor($r/18)) % 4]))]	;#send rate during on time (bps) 100000 , 1600000 , 5000000 , 20000000 video 
set val(packetSize) [expr ($ps([expr $r % 3]))]               ;# Set Packet Size to 300 bytes , 1000 , 1400
...
set filename "/mnt/sda3/results/3/results/$r$val(controller)$val(agent)results.csv"
...
proc stop {} {
        ...
        puts $resultsf "Run_id application_Generation_Rate(bps) packet_Size(bytes) agent #nodes #Connections Distance(m) connection_id from to  avg_Delay(sec) avg_rate(Mbps) avg_lost_ratio%(%packets) sent(bytes) received(bytes) total_time(sec) count Energy_per_bit(wpb)" 
	for {set i 0} {$i < $val(numberOfConnections) } {incr i} {
          ...
          set np($i) [$sink($i) set npkts_]
            ...
            #  Watt per bit
            #
            puts $resultsf "$r $val(packetGenerationRate) $val(packetSize) $val(agent) $val(nn) $val(numberOfConnections) NA $i $from $to  [expr ($avgDelay($i)/$count($i))] [expr (($totalReceivedBytes($i) * 8)/($totalTime($i) * 1000000))] [expr ($totalLost($i)/($np($i)+$totalLost($i))* 100)] NA $totalReceivedBytes($i) $totalTime($i) $count($i) [expr ($totalEn($to) / ($totalReceivedBytes($i) * 8))]" 
            ...
            set networkDeliveryRatio [expr ($networkDeliveryRatio + [expr ($totalLost($i)/($np($i)+$totalLost($i))* 100)])]
  ...
  puts $resultsf "Run_id application_Generation_Rate(bps) packet_Size(bytes) agent #nodes #Connections Distance(m) node_id BT_energy(%mW) AMP_energy(%mW) total_energy_consumption(%mW)"       
	for {set i 0} {$i < $val(nn) } {incr i} {
        	close $nodeTrace($i) 
	...
	set bb($i) [$node_($i) set bb_]
          ...
          set btEn($i) [expr (($val(initialenergy) - $btEn($i))/$val(initialenergy) * 100)]
          #amp Energy
          set ampEn($i) [$node_($i) ampEnergy]
          set totalEn($i) [expr ($totalEn($i) - $ampEn($i))]
          set ampEn($i) [expr (($val(initialenergy) - $ampEn($i))/$val(initialenergy) * 100)]
		
          set totalEn($i) [expr (($totalEn($i))/$val(initialenergy) * 100)]
          set networkEnergyConsumption [expr ($networkEnergyConsumption + $totalEn($i))]
          
          
          puts $resultsf "$r $val(packetGenerationRate) $val(packetSize) $val(agent) $val(nn) $val(numberOfConnections) NA $i $btEn($i) $ampEn($i) $totalEn($i)" 
    ...
    puts $resultsf "Run_id application_Generation_Rate(bps) packet_Size(bytes) agent #nodes #Connections Distance(m) avg_NW_Delay(sec) avg_NW_rate(Mbps) avg_NW_lost_ratio(%packets) NW_sent(bytes) NW_received(bytes) NW_Energy_per_bit(wpb) NW_total_energy_consumption(%mW)"      
    puts $resultsf "$r $val(packetGenerationRate) $val(packetSize) $val(agent) $val(nn) $val(numberOfConnections) NA $networkDelay $networkThroughput $networkDeliveryRatio NA $networkBytesReceived $networkEnergyPerBit $networkEnergyConsumption" 
