...
Simulator instproc finish {} {
	global ns
	global f
	global rnc bs ue NUM_USERS NUM_CELLS ftp_users
	$ns flush-trace
	close $f
	global src_agent src
	# tcp trace file:
	...
	close $f_tcp
	for { set i 0 } { $i < $ftp_users } { incr i } {
		#$ftpsrc($i) start
		$src_agent($i) print_tcp_traced_var $tcptracefile
	}
	...
	puts $f_flow_control "cell\tcap_req\tcap_alloc\ttotal_stallings\tstalling_fraction\thsdsch_utilization\tprob_empty_buffer"
	close $f_flow_control	
	set num_stacks [expr $NUM_CELLS * 2]
	...
	set f_mac [open $mac_file w]
	puts $f_mac "node\tmac_tx\tmac_rtx\tmac_ttx\tmac_rx\tmac_rrx\tmac_trx\tmac_erx\tmac_trxttx\trlc_rx\trlc_tx\trlc_dropped"
	close $f_mac

	set cbr_exp_pkt_stats_file "temp/cbr_exp_packet_stats.tr"
	set f_cbr_exp [open $cbr_exp_pkt_stats_file w]
	puts $f_cbr_exp "node\ttype\tpacket_rx\tdiscards\tbytes_rx\tgood_bytes_rx\tsum_delay\tavg_jitter"
	close $f_cbr_exp
	...
	for { set i 0 } { $i < $NUM_USERS } { incr i } {
		...
		if {$i >= $ftp_users} {
			$ue($i) print_cbr_exp_packet_stats $cbr_exp_pkt_stats_file
		}
	}
...
if {$FLOW_CONTROL_MODE == 6 || $FLOW_CONTROL_MODE == 7} {
	...
	Mac/Hsdpa set fc_update_interval_	30ms
...
# RLC UM-HS entity settings: used at RNC only
UMTS/RLC/UMHS set buffer_level_max_ $RLC_BUFFER_SIZE 
UMTS/RLC/UMHS set flow_max_ $NUM_USERS
UMTS/RLC/UMHS set priority_max_ 1
UMTS/RLC/UMHS set credit_allocation_interval_ $CREDIT_ALLOCATION_INTERVAL

UMTS/RLC/UMHS set flow_control_enhanced_ $ENHANCED_FLOW_CONTROL
UMTS/RLC/UMHS set flow_control_disabled_ $FLOW_CONTROL_DISABLED
if {$FLOW_CONTROL_MODE == 5} {
	UMTS/RLC/UMHS set flow_control_from_bs_ 1
	Mac/Hsdpa set Rdef_	100kb
	Mac/Hsdpa set Tu_	30ms
	Mac/Hsdpa set a_	1.0
	Mac/Hsdpa set Tm_	50ms
...
# initial credits: must match between UMHS & Hsdpa
UMTS/RLC/UMHS set initial_credits_ 20
#/UMM: timer to hold down capacity-requests for a short interval while packets are arriving at RNC
# to avoid sending multiple requests
UMTS/RLC/UMHS set cap_req_holddown_interval_ $CAP_REQ_HOLDDOWN_INTERVAL

UMTS/RLC/UMHS set Iub_frame_period_	1000us
...
#for Necker's Flow Control AL=lgorithm
Mac/Hsdpa set Tw_ 0.05
...
# turn on multicell configuration
Mac/Hsdpa set multi_cells_ 1
...
# setup the Hsdpa cell in this BS, create AM-HS, UM-HS, MAC-HS & PHY entities
for {set i 0} {$i<$NUM_CELLS} { incr i } {
	# enable mixed AM & UM mode
...
# in mixed_mode, set 50% of the users to TCP over AM and the other 50% to EXP over UM
set ftp_users [expr $NUM_USERS / 2]
for {set i 0} {$i < $NUM_USERS} {incr i} {
	if {$i < $ftp_users} {
		$ue($i) set ue_llType_ UMTS/RLC/AM
	} else {
		$ue($i) set ue_llType_ UMTS/RLC/UM
	}
}
...
for {set i 0} {$i<$NUM_USERS} { incr i } {
	if {$i < $ftp_users} {
		...
		set src_agent($i) [new Agent/TCP/Newreno]
	#	set src_agent($i) [new Agent/TCP/FullTcp/Newreno]
		$src_agent($i) set fid_ $i
		$src_agent($i) set prio_ 0
		set sink($i) [new Agent/TCPSink]
	#	set sink($i) [new Agent/TCP/FullTcp/Newreno]
	#	$sink($i) listen
		
		#FTP sources
		set src($i) [new Application/FTP]
		$src($i) attach-agent $src_agent($i)

	} else {
		#UDP transport agents	
		set src_agent($i) [new Agent/UDP]
		$src_agent($i) set fid_ $i
		$src_agent($i) set prio_ 0
		set sink($i) [new Agent/Null]
	
		#Exponential On/Off sources
		set tmp_rng [new RNG]
		# set the RNGs to the correct substream, use the iteration number to
		# seed the rng so that we can re-produce the same results for each iteration
		for {set j 1} {$j < $ITER} {incr j} {
		...
		set src($i) [new Application/Traffic/Exponential]
		$src($i) use-rng $tmp_rng
		$src($i) set packetSize_ 40
		$src($i) set burst_time_ 3
		$src($i) set idle_time_ 3
		$src($i) set rate_ 64k
		$src($i) attach-agent $src_agent($i)
	}
	...
	$ns attach-agent $server($k) $src_agent($i)

	$ns attach-agent $ue($i) $sink($i)
	$ns connect $src_agent($i) $sink($i)
