...
while {$i < $argc} {
	switch [lindex $argv $i] {
		...
		f3 {
			set arguments(-f3start) "[lindex $argv [expr {$i + 1}]]"
			set arguments(-f3stop) "[lindex $argv [expr {$i + 2}]]"
			set i [expr {$i + 2}]
			set have(-f3) "true"
		}
		distance {
...
if {$have(-f3) == "false"} {
	set arguments(-f3start) 0
	set arguments(-f3stop) 16e5
}
if {$have(-distance) == "false"} {
	set arguments(-distance) 200
}

set arguments(-scenarioname) "twopairs.$arguments(-f1).$arguments(-f2).$arguments(-distance).realbw"
...
$ns attach-agent $node_(1) $udp_(5)
set recv_(6) [new Agent/LossMonitor]
$ns attach-agent $node_(2) $recv_(6)
...
proc record_ {} {
	...
	set sortie "$now\t[$cbr_(5) set rate_]\t"
	foreach item [array names bw_] {
...
controlflux $cbr_(3) 5.5 [expr $val(stop)-5.5]
...
proc increase_find {starttime stoptime i} {
	...
	set ns [Simulator instance]
	set cbr5rate [expr $arguments(-f3start)+$i*($arguments(-f3stop)-$arguments(-f3start))/20]
	$cbr_(5) set rate_ $cbr5rate

	controlflux $cbr_(5) $starttime $stoptime

	$ns at $stoptime "$cbr_(5) set rate_ 0"
