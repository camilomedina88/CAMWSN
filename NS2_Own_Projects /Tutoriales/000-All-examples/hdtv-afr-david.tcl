...
#    $ns_ flush-trace
    ...
    set f_throughput [open "hdtv_afr_throughput.txt" "a"]
    set f_a_delay [open "hdtv_afr_average_delay.txt" "a"]
    set f_p_delay [open "hdtv_afr_peak_delay.txt" "a"]
    set f_fairness [open "hdtv_afr_fairness.txt" "a"]
	...
	for {set i 0} {$i < $val(nn) } {incr i} {
        ...
        set SizeOfAllFrame  [$mac set SizeOfAllFrame_]
...
#	puts "--------------------------------------------------------"
    set s_throughput [expr $sum/$val(stop)]
    set ind_throughput [expr $s_throughput/$val(nn)]
    puts "The system throughput = $s_throughput"
    puts "The average throughput = $ind_throughput"
    puts -nonewline $f_throughput "[string range $ind_throughput 0 6] "
...
#    puts "The average frame len = [expr $SizeOfAllFrame/$recvNumber] bytes"
