...
#Variables:
#tcps tcpr:  Sender XCP, Receiver XCP 
#sn   dn  :  source/dest node which XCP sender/receiver exist
...
XCP_pair instproc init {args} {
    $self instvar pair_id group_id id debug_mode
    $self instvar tcps tcpr;# Sender XCP,  Receiver XCP

    eval $self next $args

    $self set tcps [new Agent/TCP/XCP]  ;# Sender XCP
    $self set tcpr [new Agent/XCPSink]  ;# Receiver XCP
...
XCP_pair instproc set_debug_mode { mode } {
    $self instvar debug_mode
    $self set debug_mode $mode
}


XCP_pair instproc setup {snode dnode} {
...
XCP_pair instproc create_agent {} {
    $self instvar tcps tcpr;# Sender XCP,  Receiver XCP
    $self set tcps [new Agent/TCP/XCP]  ;# Sender XCP
    $self set tcpr [new Agent/XCPSink]  ;# Receiver XCP
}

XCP_pair instproc setup_wnode {snode dnode link_dly} {
...
XCP_pair instproc start { nr_pkts } {
    global ns
...
XCP_pair instproc flow_finished {} {
    global ns
...
Agent/TCP/XCP instproc set_callback {tcp_pair} {
    $self instvar ctrl
    $self set ctrl $tcp_pair
}


Agent/TCP/XCP instproc done {} {
    global ns sink
    $self instvar ctrl
...
#$apair($i) setup_wnode $snode $dnode 0.00001  ;# 
...
add-packet-header Flags IP TCP CC1 ;#hdrs reqd for XCP traffic
...
if {$argc != 8} {
    puts "usage: ns xxx.tcl numflows link_rate(Gbps) RTT(per hop,sec) load numbneck init_nr_flows minPkts maxPkts(1 pkt = 1000B) "
        exit 0
}
...
set minPkts [lindex $argv 6]
set maxPkts [lindex $argv 7]

set mean_npkts [expr ($maxPkts + $minPkts)/2]

puts "Simulation input:"
puts "XCP Single bottleneck"
...
puts "minPkts $minPkts pkts"
puts "maxPkts $maxPkts pkts"
puts "mean_npkts $mean_npkts pkts"
puts " "
...
#Only for XCP
Agent/TCP/XCP set packetSize_ [expr $pktSize + 40]
set xcpwin_ [expr 4 * int (ceil (($mean_link_delay * 2.0 * $link_rate * 1000000000.0/8.0) / ($pktSize+40.0)))]
Agent/TCP/XCP set window_ $xcpwin_
puts "Agent/TCP/XCP set window_ $xcpwin_"
...
$ns duplex-link $n0 $n1    [set link_rate]Gb $mean_link_delay DropTail/XCP
set bnecklink [$ns link $n0 $n1] 

#############################################################
#Only for XCP
#must set capacity for each queue 
#############################################################
set l0 [$ns link $n0 $n1]
set q0 [$l0 queue]
$q0 set-link-capacity-Kbytes [expr $link_rate * 125000.0]
set l1 [$ns link $n1 $n0]
set q1 [$l1 queue]
$q1 set-link-capacity-Kbytes [expr $link_rate * 125000.0]

#set xcplog [open xcp_status.tr w]
#$q0 queue-attach [set xcplog] 

#############  Agents          #########################
set agtagr0 [new Agent_Aggr_pair]
puts "Creating initial $init_nr_flow agents ..."; flush stdout

$agtagr0 setup $n0 $n1 0 $init_nr_flow "XCP_pair"
