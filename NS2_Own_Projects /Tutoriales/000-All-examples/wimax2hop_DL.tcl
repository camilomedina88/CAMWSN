...
#open file for NAM trace
#set nf [open $output_dir/out.nam w]
#$ns namtrace-all $nf
...
# Create God
create-god [expr ($nb_mn + 2 + $nb_rs)]				;# nb_mn + 2 (base station and sink node)
...
lappend eilastlevel 1 [expr ($nb_mn+$nb_rs+1)] 		;# number of nodes for each cluster (1 for sink and one for mobile nodes + base station
...
set rstation [$ns node 1.0.1]
$rstation set X_ 340.0
$rstation set Y_ 550.0
$rstation set Z_ 0.0
[$rstation set mac_(0)] set-channel 1
...
for {set i 0} {$i < $nb_mn} {incr i} {
	set wl_node_($i) [$ns node 1.0.[expr $i + 1 + 1]] 	;# create the node with given @.
	$wl_node_($i) random-motion 0			;# disable random motion
	$wl_node_($i) base-station [AddrParams addr2id [$rstation node-addr]] ;#attach mn to basestation
