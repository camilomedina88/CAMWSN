...
Application/LEACH instproc advertiseClusterHead {} {
  ...
  $self instvar currentCH_ code_ beginningE_
  ...
  set datasize [expr $BYTES_ID * [llength $msg] - 20]
  
  # Send beacons opt(max_dist) meters so all nodes can hear.
  # pp "Cluster Head $currentCH_ broadcasting ADV of length [expr $datasize + 20] over distance at $opt(max_dist) time [$ns_ now]"
...
#  pp "$nodeID: sending Join-REQ to $chID (dist = $dist_) at time [$ns_ now]"
  ...
  # set datasize [expr $spreading_factor * $BYTES_ID * [llength $msg]] // Deepa
  set datasize [expr $BYTES_ID * [llength $msg] - 20]
  $self WakeUp 
  
  #  pp "$nodeID: sending Join-REQ of length [expr $datasize  + 20] (length of msg = [llength $msg]) to $chID (dist = $dist_) at time [$ns_ now]"
...
Application/LEACH instproc createSchedule {} {
  ...
  if {$numNodes == 0} {
    ...
    set datasize [expr $spreading_factor * $BYTES_ID * [llength $xmitOrder] - 20]
    # pp "$chID sending TDMA schedule: $xmitOrder at time [$ns_ now]"
    pp "$chID sending TDMA schedule: $xmitOrder at time [$ns_ now] over dist = $dist_ of size [expr $datasize + 20]"
    # pp "Packet size is [expr $datasize + 20], size of msg = [expr $BYTES_ID * [llength $xmitOrder]], spreading factor = $spreading_factor. dist = $dist_ "
    set mac_dst $MAC_BROADCAST
    set link_dst $chID
...
Application/LEACH instproc recvDATA {msg} {
  ...
  if {$nodeID == [lindex $TDMAschedule_ $last_node]} {
    ...
    # pp "\tcompute_energy = $compute_energy sig_size = $opt(sig_size) num_sigs = $num_sigs"
...
Application/LEACH instproc sendData {} {
  ...
  set datasize [expr $spreading_factor * \
               [expr [expr $BYTES_ID * [llength $msg]] + $opt(sig_size)] - 20]

  $self WakeUp 

  pp "$nodeID sending data $msg of length [expr $datasize + 20] (msg = [expr $BYTES_ID * [llength $msg]], signal = $opt(sig_size)) to $currentCH_ at [$ns_ now] (dist = $dist_)"
  set mac_dst $MAC_BROADCAST
  ...
  # else {
  #pp " DATA Cant BE SENT AS  [expr $xmitat + $end_frm_time_] (xmitat = $xmitat, end_frm_time_ = $end_frm_time_) is less than [expr $next_change_time_ - 10 * $opt(ss_slot_time)] (next_change_time_ = $next_change_time_, ss_slot_time = $opt(ss_slot_time))"
  # }
...
Application/LEACH instproc SendDataToBS {} {
      ...
      $self instvar code_ rng_ now_ beginningE_
      ...
      set datasize [expr $spreading_factor * \
                         [expr $BYTES_ID * [llength $msg] + $opt(sig_size)] - 20]      
      ...
      #puts "Node $nodeID sending $msg to BS at time $random_delay"
      #puts "Node $nodeID sending to BS data $msg of length [expr $datasize + 20] (length of msg =  [llength $msg] , sig size = $opt(sig_size)) at $random_delay (dist = $dist)"
...
Application/LEACH instproc SendMyDataToBS {} {
      global ns_ opt
      $self instvar next_change_time_ alive_ dataReceived_   			
      
      set node_ID [$self nodeID]			    				
      set dataReceived_ $node_ID			     				
      # pp "in ns-leach.tcl -> SendDataToBS, dataReceived_ = $dataReceived_"	
