############################################################################
# ZCJ DHAC 2007
#   This code is based on the LEACH protocol. (June, 2000)
#   Developed as part of the WSNs project at Carleton University.
############################################################################

global opt bs

source dhac/tools/ns-ranode.tcl
source dhac/tools/ns-bsapp.tcl
source dhac/tools/algorithmtools.tcl
source dhac/tools/stats.tcl
source dhac/tools/ns-resource-manager.tcl
source dhac/tools/ns-energy-resource.tcl
source dhac/tools/ns-neighbor-resource.tcl
...
set opt(nn_alive)     [expr $opt(nn) - 1] ;# Number of alive nodes
...
set opt(slot_time)    [TxTime [expr $opt(sig_size)+$opt(hdr_size)]]
# cluster packet transmission time
set opt(ss_slot_time) [expr $opt(slot_time) * $opt(num_clusters)]
# Maximum TDMA frame time
set opt(frame_time)   [expr $opt(ss_slot_time) * $opt(nn_)]
###zcj: important setting time for each round 
set opt(step_span)    [TxTime [expr $opt(sig_size)+$opt(hdr_size)]]
set opt(ch_change)    [expr 10 * $opt(init_energy)]  ;# Time for each round


set opt(check_energy)  5                  ;# Time btwn energy traces 
...
Phy/WirelessPhy set ss_            $opt(nn_);#zcj changed spreading to nn
Phy/WirelessPhy set dist_          75
...
Mac/Sensor set ss_                 $opt(nn_);#zcj changed spreading to nn
...
proc leach-create-mobile-node { id } {
    ...
    global Neighbor_ matrix_ cluster_;#zcj
...
proc sens_init {} {
    ...
    catch "exec rm $opt(dirname)/$opt(filename).datanode"
    catch "exec rm $opt(dirname)/$opt(filename).end"
    catch "exec rm $opt(dirname)/$opt(filename).stat"
    
    catch "exec rm $opt(dirname)/$opt(filename).msg"
    pp -file "msg" "steps\ttime\tname\tsender\treciever\tmessageID\tcontent\tdatasize\tdistance\tmac_dst\tlink_dst" 
    catch "exec rm $opt(dirname)/$opt(filename).neighbor"
    catch "exec rm $opt(dirname)/$opt(filename).cluster"
    catch "exec rm $opt(dirname)/$opt(filename).matrix"
    catch "exec rm $opt(dirname)/$opt(filename).merge"
    catch "exec rm $opt(dirname)/$opt(filename).startup"
    catch "exec rm $opt(dirname)/$opt(filename).temp"
    catch "exec rm $opt(dirname)/$opt(filename).maintain"
