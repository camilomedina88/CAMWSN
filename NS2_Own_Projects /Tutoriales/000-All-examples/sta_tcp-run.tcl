...
set Frag    {1024 1024 1024}
#set Frame   {4096 4096 4096}
#set Frag    {256}
set nn      {10 20 30 50 80}
set x_axis  $nn

set fh_throughput "sta_tcp_afr_throughput.txt"
set fh_a_delay    "sta_tcp_afr_average_delay.txt"
set fh_p_delay    "sta_tcp_afr_peak_delay.txt"
set fh_fairness   "sta_tcp_afr_fairness.txt"
...
for {set i 0} {$i < [llength $BER] } {incr i} {
    for {set j 0} {$j < [llength $nn] } {incr j} {
   	    exec ns sta_tcp-afr-david.tcl [lindex $BER $i] [lindex $PKT $i] [lindex $Frame $i] [lindex $Frag 0] [lindex $DelayTimer 0]  [lindex $nn $j] >> "auto-result.txt" 2> /dev/null
    }
    new_a_line
}
