...
Mac/802_16 set min_th_ 3 ;# MR_min padrão 1
Mac/802_16 set max_th_ 9 ;# MR_max padrão 2
...
WimaxScheduler/BS set quantum_ 1000
WimaxScheduler/RS set quantum_ 1000
WimaxScheduler/SS set quantum_ 1000
...
# Open file for trace
set tf [open $output_dir/wimax_1RS_2SS_2SN_mod_05.tr w]
$ns trace-all $tf
#puts "Output file configured"

# Open file for recording throughput
set throughput_rv_ [open $output_dir/throughput_mod_05.tr w]
...
#[$bstation set mac_(0)] set-servicehandler [new ServiceFlowHandler]
...
for {set i 0} {$i < $nb_rs} {incr i} {
  ...
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 10.0 20.0 2 6    ;# start; end; MS address; DIUC
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 20.0 30.0 2 5
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 30.0 40.0 2 4
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 40.0 50.0 2 3
  [$rstation_($i) set mac_(0)] add-ms-period-diuc 50.0 60.0 2 2
  #[$rstation_($i) set mac_(0)] add-ms-period-diuc 70.0 80.0 2 1
  #[$rstation_($i) set mac_(0)] add-ms-period-diuc 10.0 20.0 3 7    ;# start; end; MS address; DIUC
  #[$rstation_($i) set mac_(0)] add-ms-period-diuc 20.0 30.0 3 2
  #[$rstation_($i) set mac_(0)] add-ms-period-diuc 30.0 40.0 3 1
  #[$rstation_($i) set mac_(0)] add-ms-period-diuc 40.0 50.0 3 2
  #[$rstation_($i) set mac_(0)] add-ms-period-diuc 50.0 60.0 3 7
...
for {set i 0} {$i < $nb_mn} {incr i} {
  ...
  #[$wl_node_($i) set mac_(0)] set-scheduler [new WimaxScheduler/SS]
  #[$wl_node_($i) set mac_(0)] add-classifier [new SDUClassifier/Dest]
...
#  if {$i == 0} {
#  	[$wl_node_($i) set mac_(0)] add-ms-period-diuc 10.0 20.0 1 7    ;# start; end; MS address; DIUC
#  	[$wl_node_($i) set mac_(0)] add-ms-period-diuc 20.0 30.0 1 2
#  	[$wl_node_($i) set mac_(0)] add-ms-period-diuc 30.0 40.0 1 1
#  	[$wl_node_($i) set mac_(0)] add-ms-period-diuc 40.0 50.0 1 2
#  	[$wl_node_($i) set mac_(0)] add-ms-period-diuc 50.0 60.0 1 7
#  } else {
#	[$wl_node_($i) set mac_(0)] set-diuc 7   ;# Change the node profile here (7=64QAM_3_4)
#  }
  # Ordem 7 -> 2 -> 1 -> 2 -> 7
  # Ordem 10 -> 20 -> 30 -> 40 -> 50
  #$ns at 10.0 "[$wl_node_($i) set mac_(0)] set-diuc 7"
  #$ns at 20.0 "[$wl_node_($i) set mac_(0)] set-diuc 2"
  #$ns at 30.0 "[$wl_node_($i) set mac_(0)] set-diuc 1"
  #$ns at 40.0 "[$wl_node_($i) set mac_(0)] set-diuc 2"
  #$ns at 50.0 "[$wl_node_($i) set mac_(0)] set-diuc 7"
  ...
  #$cbr_($i) set rate_ [expr 500*8*1024]
  #$cbr_($i) set interval_ 0.25
  #$cbr_($i) set random_ 1
  #$cbr_($i) set maxpkts_ 1000000
  #$cbr_($i) set rate_ 500
